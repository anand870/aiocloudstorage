...
import pytest
import aiobotocore
from aiocloudstorage import Container,configure
...
async def test_blob_file_url_on_create(container,text_filename):
    blob = await container.upload_blob(text_filename,blob_name=TEXT_FILENAME)
    assert blob.file_url == FILE_URL%('minio',container.name,TEXT_FILENAME)
...
async def test_blob_file_url_on_get(container,text_filename):
    ...
    assert fetch_blob.file_url == FILE_URL%('minio',container.name,TEXT_FILENAME)
